...
其实就是一句话，Java虚拟机有预加载功能。  下方在第**3、初始化**[点击跳转](#jump)中讲解了必须要触发类进行加载的时机。
...
虚拟机规范则严格规定了有且只有四种情况必需立即对类进行“初始化”，也就是<span id="jump">加载时机</span>，（而加载、验证、准备阶段则必需在此之前开始），这四种情况归类如下：
...
[参考博客](http://developer.51cto.com/art/201103/249613.htm)
