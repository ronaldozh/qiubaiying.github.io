...
+ 1.在plugin 中安装
+ 2.[访问github中lanyus大神文章](http://github.com/ilanyu/ReverseProxy/releases/tag/v1.4)，下载自己机器系统相对应的工具
+ 3.完成之后，直接双击打开，使用本机8888端口
+ 4.直接进入到jrebel的配置项中，点击active now链接进入到激活界面
+ 5.Tewam URL填写为http://127.0.0.1:8888/guid,guid用guid生成工具生成，下面的邮箱随意填写
+ 6.点击注册，点击work offline,离线工作


+ [参考：JRebel永久破解激活方法](https://www.cnblogs.com/jwcz/p/10997384.html)
+ [参考：Create GUIDs online](https://www.guidgen.com/)
...
+ **RestfulToolkit:** 根据路径名查询restful api的地址，使用时ctrl+alt+n,弹出框搜索。
...
+ 如果plugins中的marketplace无法查询任何插件，则需要Setting–》Appearance & Behavior --》Syetem Setting --》Http Proxy 选择Auto-detect proxy settings，重启idea
+ 快捷键设置复制上一行：File->settings->keymap->搜索duplicate ->双击Duplicate Entire Lines，设置ctrl+向下箭头，如果是英特尔主板则默认是ctrl+alt+向下箭头 复制
